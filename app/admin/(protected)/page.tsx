...
export default function AdminDashboard() {
  ...
  return <main className="admin-main">
    ...
    <div className="stats">
      ...
      <div className="stat"><small>Faol mahsulot</small><strong>{s.active}</strong></div>
    ...
    <section className="admin-card">
      ...
      <div className="table-wrap"><table><thead><tr><th>Mahsulot</th><th>Ko‘rish</th><th>Buyurtma</th><th>Konversiya</th><th></th></tr></thead><tbody>
        {top.map(p => <tr key={p.id}><td><div className="inline"><Image className="product-mini" src={p.image_url} alt="" width={50} height={50}/><b>{p.name}</b></div></td><td>{p.views}</td><td>{p.orders}</td><td>{p.conversion}%</td><td><Link href={`/p/${p.slug}`} target="_blank">Sahifa ↗</Link></td></tr>)}
      </tbody></table></div>
    </section>
  </main>;
}
